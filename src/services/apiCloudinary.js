...
export async function fetchPreviewProjects() {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      data = dataJSON;
      // JSON file should contain the latest from : https://res.cloudinary.com/dayojijed/image/list/yhr-project.json
    } else {
      const response = await fetch(`${API_BASE_URL}/yhr-project.json`);
      if (!response.ok) {
